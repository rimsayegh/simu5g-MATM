...
void ServiceRegistry::registerMecService(const ServiceDescriptor& servDesc)
{
     ...
     */
    EV << "ServiceRegistry::Rim - "<< servDesc.name.c_str()<< "Hela" << endl;
    ...
    /*SRFile.open("serviceR.txt", std::ios::app);

    if (SRFile.is_open()) {
          SRFile << "the mec services vector: "<<  mecServices_.size() << std::endl;
          for (size_t i = 0; i < mecServices_.size(); ++i) {
                          ServiceInfo service = mecServices_[i];
                          SRFile << "Service " << i + 1 << ": " << service.getName() << std::endl;
                      }
          SRFile.close();
      }*/
