...
void LocationService::initialize(int stage)
{
    ...
    if (stage == inet::INITSTAGE_APPLICATION_LAYER) {
        ...
        //L2S-ESME
        omnetpp::cModule* platform = getParentModule();  // MECPlatform is the parent of LocationService

        omnetpp::cModule* MECmodule = platform->getParentModule();

        logFile.open("output.txt", std::ios::app);  // Open file for appending
        if (!logFile.is_open()) {
            EV << "Error opening log file!" << endl;
            return;
        }
        logFile << "Simulation started at " << simTime() << std::endl;
        logFile << "The location service is in the mecHost: "<< MECmodule << std::endl;
        logFile.close();
        //L2S-ESME
    }
...
bool LocationService::manageSubscription()
{
    ...
    if(subscriptions_.find(subId) != subscriptions_.end())
    {
        ...
        if (locationFile.is_open()) {
            ...
            locationFile << "The subscription Id: "<< subId << std::endl;
            locationFile.close();
        }
        //L2S-ESME
        omnetpp::cModule* platform = getParentModule();  // MECPlatform is the parent of LocationService
        ...
        logFile.open("mecapp.txt", std::ios::app);  // Open file for appending

        if (logFile.is_open()) {
            logFile << "Manage Subscription in the mecHost: "<< MECmodule << "at time: "<< simTime() << std::endl;
           logFile.close();
        }
...
//L2S-ESME
void LocationService::sendMigrationNotification(int vehicleId) {
    // Retrieve the parent MECPlatform module
    omnetpp::cModule* platform = getParentModule();  // MECPlatform is the parent of LocationService

    omnetpp::cModule* MECmodule = platform->getParentModule();
    // Retrieve the mecOrchestrator submodule from the MECPlatform
    omnetpp::cModule* platformManager = MECmodule->getSubmodule("mecPlatformManager");
    ...
    if(platformManager != nullptr)
       {
           ...
           MecPlatformManager* mecPlatformManager = check_and_cast<MecPlatformManager*>(platformManager);
           if(mecPlatformManager!= nullptr){

               EV<< vehicleId<<"mecPlatformManager->sendMigrationNotification(vehicleId)"<<endl;
               /*logFile.open("mecapp.txt", std::ios::app);  // Open file for appending

               if (logFile.is_open()) {
                   logFile << "The migration detected by the location service in the mecHost: "<< MECmodule << std::endl;
                   logFile.close();
               }*/

               logFile.open("migration.txt", std::ios::app);  // Open file for appending

               if (logFile.is_open()) {
                  logFile << "The migration of vehicle: "<< vehicleId <<" started at: "<< simTime() << std::endl;
                  logFile.close();
               }
               mecPlatformManager->sendMigrationNotification(vehicleId);

           }
       }
    else {
           EV << "Error: mecOrchestrator not found!" << endl;
       }
}
...
//    }
    ...
    if(baseUri.compare(baseUriSubscriptions_+"/area/circle") == 0)
    {
       ...
       if(it != subscriptions_.end())
       {
          ...
          if (logFile.is_open()) {
              logFile << "Test:"<< sub->getActionCriteria() << std::endl;
              logFile.close();
          }*/

          //L2S-ESME
          if(sub->getActionCriteria() == "Yes") //if error use strcmp
          {
                ...
                if (logFile.is_open()) {
                    logFile << "LocationService::handlePUTRequest - Migration" << std::endl;
                    logFile << "LocationService::handlePUTRequest - Vehicle app ID is : "<< VehID << std::endl;
                    /*logFile << "LocationService::handlePUTRequest - the vehicle enters : "<< sub->getcount()<< "times" << std::endl;*/
                    logFile.close();
                }


                /*locationFile.open("Vehicle.txt", std::ios::app);  // Open file for appending

                if (locationFile.is_open()) {

                    locationFile << "The vehicle location is: "<< sub->getcount()<< std::endl;
                    locationFile.close();
                }*/



                sendMigrationNotification(VehID);


          }
          if(sub->getActionCriteria() == "No")
          {
              EV << "Subscription is for Leaving event." << endl;
              ...
              if (logFile.is_open()) {
                  logFile << "LocationService::handlePUTRequest - Nooooooooooo Migration" << std::endl;
                  logFile << "LocationService::handlePUTRequest - Vehicle app ID"<< VehID << std::endl;
                  logFile.close();
              }
