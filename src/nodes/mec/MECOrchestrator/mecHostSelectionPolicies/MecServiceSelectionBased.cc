...
cModule* MecServiceSelectionBased::findBestMecHost(const ApplicationDescriptor& appDesc)
{
    ...
    ResourceDescriptor resources = appDesc.getVirtualResources();
    USFile.open("Util.txt", std::ios::app);  // Open file for appending
    if (USFile.is_open()) {
        USFile << "The : " << bestHost << " , "<< " CPU "  << resources.cpu << std::endl;
        USFile << "The : " << bestHost << " , "<< " RAM "  << resources.ram << std::endl;
        USFile << "The : " << bestHost << " , "<< " Disk "  << resources.disk << std::endl;
        USFile.close();
    }
