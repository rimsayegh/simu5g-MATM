...
std::ofstream RsFile_;
std::ofstream UTsFile_;
std::ofstream Track_;
std::ofstream csvFile_;

std::vector<double> pastLBDs_; // Store the past LBD values globally
std::vector<double> pastLoads_;
std::vector<double> pastDistances_;
...
void writeToCSV(simtime_t time, std::string Type, double Vehicle_x, double Vehicle_y, double Speed, double DataRate,double Distance1,double Distance2,double Distance3, double Min_Distance, double Max_Distance, double MEC_Load1,double MEC_Load2, double MEC_Load3, double Min_Load, double Max_Load, double LBD,
    ...
    csvFile_.open("metrics_log.csv", std::ios::app);

    if (csvFile_.is_open()) {
        csvFile_ << std::fixed << std::setprecision(10);

        static bool headerWritten = false;
        if (!headerWritten) {
            csvFile_ << "Time,Type,Vehicle_x,Vehicle_y,Speed,DataRate,Distance1,Distance2,Distance3,Min_Distance,Max_Distance,MEC_Load,Min_Load,Max_Load,LBD,CF1,CF2,Best_MEC,alpha,betta,lamda\n";
            headerWritten = true;
        }
...
cModule* RL::findBestMecHost(const ApplicationDescriptor& appDesc)
{
    ...
    for (auto mecHost : mecOrchestrator_->mecHosts)
    {
        ...
        for (const auto& mecService : *mecServices)
        {
            if (serviceName.compare(mecService.getName()) == 0 && mecService.getMecHost().compare(mecHost->getName()) == 0)
            {
                ...
                pastLoads_.push_back(load);
                pastDistances_.push_back(distance);


                MEC.push_back(std::make_tuple(mecHost, load, distance));

                RsFile_.open("Load.txt", std::ios::app);  // Open file for appending

                if (RsFile_.is_open()) {
                    RsFile_ << std::fixed << std::setprecision(10);  // Ensures 10 decimal places
                    RsFile_ << "Time:" << simTime() << " / " <<"The mecHost: " << mecHost->getFullPath() << " has load: " << load << std::endl;
                    RsFile_.close();
                }
    ...
    if (!pastLoads_.empty()) {
               ...
               minLoad = *std::min_element(pastLoads_.begin(), pastLoads_.end());
               maxLoad = *std::max_element(pastLoads_.begin(), pastLoads_.end());

               minDistance = *std::min_element(pastDistances_.begin(), pastDistances_.end());
               maxDistance = *std::max_element(pastDistances_.begin(), pastDistances_.end());
    ...
    for (const auto& entry : MEC) {
            ...
            RsFile_.open("vector.txt", std::ios::app);

            if (RsFile_.is_open()) {
               RsFile_ << std::fixed << std::setprecision(10);  // Ensures 10 decimal places
               RsFile_ << "Time:" << simTime() << " / " <<"The mecHost: " << mecHost->getFullPath() << " has load: " << load << ", distance: " << distance << ", CF1: "<< CF1 << ", CF2: " << CF2 << std::endl;
               RsFile_.close();
            }
        }
    ...
    pastLBDs_.push_back(LBD);


    double meanLBD = calculateMean(pastLBDs_);
    double stdDevLBD = calculateStdDev(pastLBDs_, meanLBD);
    ...
    RsFile_.open("LBD.txt", std::ios::app);

    if (RsFile_.is_open()) {

        RsFile_ << std::fixed << std::setprecision(10);  // Ensures 10 decimal places
        RsFile_ << "Time:" << simTime() << " / " <<" LBD is: " << LBD << std::endl;
    ...
    if (RsFile_.is_open()) {
        ...
        RsFile_ << std::fixed << std::setprecision(10);  // Ensures 10 decimal places
        RsFile_ << "Threshold: " << LBDThreshold << std::endl;
    ...
    if (LBD > LBDThreshold) {
        ...
        Track_.open("Track_.txt", std::ios::app);

        if (Track_.is_open()) {

            Track_ << std::fixed << std::setprecision(10);  // Ensures 10 decimal places
            Track_ << "Load Aware - Best Host is: " << bestMecHost << std::endl;
...
*/
        ...
        RsFile_.open("Score.txt", std::ios::app);
        if (RsFile_.is_open()) {
            RsFile_ << "The minimum distance between vehicle and MEC host is : " << std::get<2>(bestHosts.front()) << " units" << std::endl;
            RsFile_.close();
        }
        ...
        RsFile_.open("Score.txt", std::ios::app);
        if (RsFile_.is_open()) {
            RsFile_ << "The relocation duration is : " << duration << "ms" << std::endl;
            RsFile_.close();
        }

        RsFile_.open("Score.txt", std::ios::app);  // Open file for appending

        if (RsFile_.is_open()) {
            RsFile_ << "The closest MEC Host is : " << closestMecHost << std::endl;
            RsFile_.close();
        }
        ResourceDescriptor resources = appDesc.getVirtualResources();
        UTsFile_.open("Util.txt", std::ios::app);  // Open file for appending
        if (UTsFile_.is_open()) {
            UTsFile_ << "The : " << closestMecHost << " , "<< " CPU "  << resources.cpu << std::endl;
            UTsFile_ << "The : " << closestMecHost << " , "<< " RAM "  << resources.ram << std::endl;
            UTsFile_ << "The : " << closestMecHost << " , "<< " Disk "  << resources.disk << std::endl;
            UTsFile_.close();
        }
       ...
       // vim->PrintResources();
        Track_.open("Track_.txt", std::ios::app);  // Open file for appending

        if (Track_.is_open()) {

            Track_ << std::fixed << std::setprecision(10);  // Ensures 10 decimal places
            Track_ << "Distance Aware - Best Host is: " << closestMecHost << std::endl;
