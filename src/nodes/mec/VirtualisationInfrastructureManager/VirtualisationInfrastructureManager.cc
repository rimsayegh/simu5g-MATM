...
void VirtualisationInfrastructureManager::initialize(int stage)
{
    ...
    cModule* Lservice = mecHost->getSubmodule("mecPlatform")->getSubmodule("mecService[1]");
    /*VIFile.open("output.txt", std::ios::app);

   if (VIFile.is_open()) {
       VIFile << "During the instantiate MEC app, the size of mecServices is:"<< Lservice << std::endl;
...
MecAppInstanceInfo* VirtualisationInfrastructureManager::instantiateMEApp(CreateAppMessage* msg)
{
   ...
   /* VIFile.open("output.txt", std::ios::app);
           ...
           if (VIFile.is_open()) {
               VIFile << "During the instantiate MEC app, the size of mecServices is:"<< meServices.size() << std::endl;
               for (size_t i = 0; i < meServices.size(); ++i) {
                               cModule* service = meServices[i];
                               VIFile << "Service " << i + 1 << ": " << service->getName() << std::endl;
                           }
...
//        EV << "VirtualisationInfrastructureManager::instantiateMEApp - UEAppL3Address: " << ueAppAddress.str() << endl;
        ...
        cModule *module = moduleType->create(meModuleName, mecHost); //Application Name

        /*VFile.open("vim.txt", std::ios::app);  // Open file for appending

            if (VFile.is_open()) {
                VFile << "The module in the Vim is : "<< module << std::endl;
                VFile.close();
            }*/


        //MEAPP module-name & its Parent Module
        std::stringstream appName;
...
//L2S-ESME
bool VirtualisationInfrastructureManager::terminateR(DeleteAppMessage* msg, cModule* mecHost)
{
    int ueAppID = msg->getUeAppID();
    ...
    /*VIFile.open("output.txt", std::ios::app);
         ...
         if (VIFile.is_open()) {
             VIFile << "In the Vim: "<< ueAppID << std::endl;
             VIFile.close();
         }*/
     ...
     if(!mecAppMap.empty() && mecAppMap.find(ueAppID) != mecAppMap.end())
    {
        ...
        if (VIFile.is_open()) {
            VIFile << "Vim , in the if:"<< currentMEApps << std::endl;
            VIFile.close();
        }*/
        //EV << "VirtualisationInfrastructureManager::terminateMEApp - currentMEApps: " << currentMEApps << " / " << maxMECApps << endl;
...
//        ackMEAppPacket(packet, ACK_STOP_MEAPP);
        ...
        if (VIFile.is_open()) {
            VIFile << "service Index"<< serviceIndex << std::endl;
            VIFile.close();
        }*/
        ...
        if(serviceIndex >= 0)
        {
            ...
            if (VIFile.is_open()) {
                   VIFile << "Problem with location Service"<< std::endl;
                   VIFile.close();
            }*/
        }
        else
        {
            VIFile.open("output.txt", std::ios::app);
                       ...
                       if (VIFile.is_open()) {
                              VIFile << "Problem with location Service"<< meServices.at(0) << std::endl;
                              VIFile.close();
                       }
        }
...
EV << "VirtualisationInfrastructureManager::findService " << serviceName << endl;
    ...
    if(it == meServices.end()){
        ...
        if (VIFile.is_open()) {
               VIFile << "the service index in the find service function: "<< SERVICE_NOT_AVAILABLE << std::endl;
               VIFile.close();
        }*/
...
//ESME-L2S
void VirtualisationInfrastructureManager::PrintResources(){
    cModule* MEC = getParentModule();
    VIRFile.open("vim.txt", std::ios::app);  // Open file for appending

                if (VIRFile.is_open()) {

                    VIRFile <<" Time: "<< simTime() << " / " << " MECHost: "<< MEC << " / " << " Ram " << " / " << allocatedRam  << " / " << maxRam << std::endl;
                    VIRFile <<" Time: "<< simTime() << " / " << " MECHost: "<< MEC << " / " << " Disk " << " / " << allocatedDisk << " / " << maxDisk  << std::endl;
                    VIRFile <<" Time: "<< simTime() << " / " << " MECHost: "<< MEC << " / " << " CPU " << " / " << allocatedCPU << " / " << maxCPU << std::endl;
