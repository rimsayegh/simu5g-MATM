...
void UALCMPApp::handleMessageWhenUp(cMessage *msg)
{
    if(msg->arrivedOn("fromMecOrchestrator"))
    {
        ...
        //L2S-ESME
        else if(strcmp(lcmMsg->getType(), Terminate_CONTEXT_APP ) == 0)
        {
            handleTerminateContextAppMessage(lcmMsg);
        }
        //L2S-ESME
...
void UALCMPApp::handleCreateContextAppAckMessage(UALCMPMessage *msg)
{
    ...
    //L2S-ESME
    UFile.open("ualcmp.txt", std::ios::app);  // Open file for appending

    if (UFile.is_open()) {
        UFile << "The request Id (reqSno):"<< reqSno << std::endl;
        UFile.close();
    }
    //L2S-ESME
    ...
    if(req == pendingRequests.end())
    {
        ...
        //L2S-ESME
         ...
         if (UFile.is_open()) {
             UFile << "req?:"<< reqSno << std::endl;
             UFile.close();
         }
         //L2S-ESME
         unsigned int reqSno = (ack->getRequestId())-1;
         auto req = pendingRequests.find(reqSno);
        //return;
    }
...
//            jsonBody["appInfo"]["userAppInstanceInfo"]["appLocation"]; // TODO not implemented yet
            ...
            /* UFile.open("ualcmp.txt", std::ios::app);  // Open file for appending
             ...
             if (UFile.is_open()) {
                 UFile << "Handle create context App Ack (in ualcmp), getSuccess?:"<< ack->getSuccess() << std::endl;
                 UFile.close();
             }*/
             //L2S-ESME
...
void UALCMPApp::handleDeleteContextAppAckMessage(UALCMPMessage *msg)
{
    ...
    if (UFile.is_open()) {
        UFile << "Test Delete : "<< reqSno << std::endl;
        UFile.close();
    }*/
    //L2S-ESME
...
//L2S-ESME
void UALCMPApp::handleTerminateContextAppMessage(UALCMPMessage *msg)
{
    TerminateContextAppMessage* ack = check_and_cast<TerminateContextAppMessage*>(msg);

    int con = ack->getContextId();
    ...
    if (UFile.is_open()) {
        UFile << "Test Terminate vehicle with contextId : "<< con << std::endl;
        UFile.close();
    }
    //L2S-ESME
...
//    // it has to be managed the case when the sub is /area/circle (it has two slashes)
    ...
    if(baseUri.compare(baseUriSubscriptions_+"/app_contexts") == 0)
    {
        ...
        if (UFile.is_open()) {
            UFile << "Handle delete request and send it to the Orchestrator. "<< std::endl;
            UFile.close();
        }*/
