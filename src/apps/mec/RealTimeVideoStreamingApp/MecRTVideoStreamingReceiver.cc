...
void MecRTVideoStreamingReceiver::handleStartMessage(cMessage* msg)
{
    ...
    simtime_t creationTime = startPkt->getTag<CreationTimeTag>()->getCreationTime();

    simtime_t startvideoDelay = simTime() - creationTime;
    emit(startVideoAppDelay_, startvideoDelay);
    startnbReceived++;
    startvideoDelaySum+=startvideoDelay;

    EV <<"] startVideoAppDelay[" << startvideoDelay << "]" << endl;
...
void MecRTVideoStreamingReceiver::handleSessionStartMessage(cMessage* msg)
{
    ...
    else
    {
        ...
        // Added by L2S/ESME
        // emit statistics
        //simtime_t startvideoDelay = simTime() - startPkt->getCreationTime();
       // emit(startVideoAppDelay_, startvideoDelay);
        //startnbReceived++;
       // startvideoDelaySum+=startvideoDelay;
