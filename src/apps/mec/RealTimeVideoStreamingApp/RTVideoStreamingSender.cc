...
void RTVideoStreamingSender::initialize(int stage)
{
    ...
    initvideoDelay_ = registerSignal("initvideoDelay");
...
void RTVideoStreamingSender::handleAckStartMECApp(cMessage* msg)
{
    ...
    if(pkt->getResult() == true)
    {
        ...
        simtime_t initVideodelay = simTime() - 2;
        emit(initvideoDelay_, initVideodelay);
        // emit(alertRcvdMsg_, (long)1);
        initvnrReceived++;
        initvideodelaySum+=initVideodelay;

        EV <<"] initDelay[" << initVideodelay << "]" << endl;
    }
    else
    {
