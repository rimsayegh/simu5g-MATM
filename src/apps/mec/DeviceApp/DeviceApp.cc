        ...
        switch(appState)
        {
            ...
            case CREATING:
            {
                ...
                if(response->getCode() == 201) // Successful response of the post
                {
                   ...
                   //L2S-ESME (change the state from Appcreated to RELOCATING)
                    appState = RELOCATING;
                    return;
                }
            ...
            case RELOCATING:
            {
                ...
                {
                    EV << "Relocation failed. Handling failure." << endl;
                    //handleRelocationFailure(response);
                }
                break;
            }
            //L2S-ESME
...
/*void DeviceApp::setState(AppState newState)
{
    EV << "Transitioning from " << appStateToString(appState) << " to " << appStateToString(newState) << endl;
    appState = newState;
}*/
//L2S-ESME
...
//L2S-ESME
/*std::string DeviceApp::appStateToString(AppState state)
{
    switch(state)
    {
        ...
        case APPCREATED: return "APPCREATED";
...
void DeviceApp::sendStopAppContext(inet::Ptr<const DeviceAppPacket> pk)
{
    ...
    if(UALCMPSocket_.getState() == inet::TcpSocket::CONNECTED && appState == APPCREATED)
    {
        ...
        DFile.open("device.txt", std::ios::app);  // Open file for appending

        if (DFile.is_open()) {
             DFile << "Send stop app in the Device App to the ualcmp, the context id is:"<< appContextUri << std::endl;
             DFile.close();
        }
    }
