...
#include "apps/mec/Safety/packets/Serializers/SafetyPacketSerializer.h"
...
Register_Serializer(SafetyAppPacket, SafetyPacketSerializer);
Register_Serializer(SafetyStartPacket, SafetyPacketSerializer);
Register_Serializer(DataPacket, SafetyPacketSerializer);


void SafetyPacketSerializer::serialize(MemoryOutputStream& stream, const Ptr<const Chunk>& chunk) const
{
    EV << "SafetyPacketSerializer::serialize" << endl;

    auto startPosition = stream.getLength();
    auto devAppPacket = staticPtrCast<const SafetyAppPacket>(chunk);
    std::string  ss = devAppPacket->getType();

    if(strcmp(ss.c_str(), START_WARNING) == 0)
    {
        auto startPk = staticPtrCast<const DataPacket>(chunk);
    ...
    {
        auto stopPk = staticPtrCast<const SafetyStopPacket>(chunk);
    ...
    {
        auto alertPk = staticPtrCast<const DataPacket>(chunk);
...
const Ptr<Chunk> SafetyPacketSerializer::deserialize(MemoryInputStream& stream) const
{
    ...
    switch (messageCode)
    {
        case 0: // START
        {
            auto startPacket = makeShared<SafetyStartPacket>();
        ...
        case 1: // STOP
        {
            auto stopPacket = makeShared<SafetyStopPacket>();
        ...
        case 2: // ALERT
        {
            auto alertPacket = makeShared<DataPacket>();
...
//            throw cRuntimeError("len %d, pos %d", B(stream.getLength()).get(),B(stream.getPosition()).get());
        ...
        case 3 : // ACK START
        {
            auto ackStartPacket = makeShared<SafetyAppPacket>();
        ...
        case 4 : // NACK START
        {
            auto nackStartPacket = makeShared<SafetyAppPacket>();
